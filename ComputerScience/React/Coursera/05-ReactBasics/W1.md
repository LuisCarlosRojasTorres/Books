...
* 2. [React Componentes and Where they live](#ReactComponentesandWheretheylive)
	...
	* 2.8. [The React project structure](#TheReactprojectstructure)
	* 2.9. [ReadingCustomizing the project](#ReadingCustomizingtheproject)
	* 2.10. [Importing components](#Importingcomponents)
	* 2.11. [Lab: Creating and importing components](#Lab:Creatingandimportingcomponents)
	* 2.12. [ReadingSolution: Creating and importing components](#ReadingSolution:Creatingandimportingcomponents)
	* 2.13. [Self review: Creating and importing components](#Selfreview:Creatingandimportingcomponents)
	* 2.14. [Knowledge check: React components and where they live](#Knowledgecheck:Reactcomponentsandwheretheylive)
	* 2.15. [ReadingAdditional resources for React components and where they live](#ReadingAdditionalresourcesforReactcomponentsandwheretheylive)
...
1. Create a React-app project `npm init react-app .`
2. run with `npm start`
3. Open src folder in EXPLORER sidebar
4. Open `App.js` and delete the code inside `App() function`. The code is presented below:
...
- So , the only way to do it is by transpiling it.
- To transpile ES6 to ES5 code yourself, using the `es6console` website.
...
export default App;
```
###  2.8. <a name='TheReactprojectstructure'></a>The React project structure
- THe Folder structure is composed of three folders:
  - `node_modules` folder:
    - The repository for all the modules in your React app.
    - It is automatically added when you install a specific `npm package`
  - `public` folder:
    - It contain the assets tha will be displayed to the user in your app. For example
      - the `favicon.ico` which displays the icon in the browser tab.
      - `robots.txt` for engine optimization
      - `manifest.json` provides some metadata to a device which runs a react app
    - THe most important file `index.html`:
      - The react app and its changes are injected to the body of this file.
  - `src` folder:
    - It contains all the essential component files. For example:
      - `index.js`, `App.js`: render the root components of the app
      - `App.css` contains the styles for the `App.js` components
      - `index.css` contains the styles to use in the entire app
      - `App.test.js`, `setupTests.js` and `reportWebVitals.js` are related to the app's performance and testing.
    - You can organize your files as you want.
    - The most important file `index.js` which imports everything that this react app needs to render 
  - `README.md` gives some information about the project
  ...
  - `package-lock.json` holds the list of all dependencies with a specific versions
###  2.9. <a name='ReadingCustomizingtheproject'></a>ReadingCustomizing the project
...
They even say that if you're just starting out, y**ou shouldn't spend more than five minutes setting up a project**.
...
**Building Components**
For now, let’s just build those components. After you've added the / folder, you’ve also added all the functional component files. Since they are all currently empty, you can start adding them, one by one.
...
###  2.10. <a name='Importingcomponents'></a>Importing components
- In order to create a fully functioning React app, you need to create a collection of components
- Modules are standalone units of code that you can reuse again and again. Being standalone means that you can add them to your programs, remove them, and replace them with other modules and everything will still work.
- The export statement is used to make a module available to another module. It helps to think of every JavaScript file as a module. Then in order to make the functions and variables available to other files, you need to export them, which makes them available by the import statement. In JavaScript, there are two types of exports, default exports and named exports. The default export is used when the function name is the same as the file name and named exports are used when you want the function name to be different from the file name. 
- what is the difference between modules and components, since they are both essentially just JavaScript files  
  - it can help to think of a component as a single part or small piece of functionality like a button. Then you can think of a module as something that's larger than just one component like a series of components
- In React, to import a component, you use the keyword import, followed by the component name you want to import. Then you use the keyword from, to specify the location of where the component is located.
...
###  2.12. <a name='ReadingSolution:Creatingandimportingcomponents'></a>ReadingSolution: Creating and importing components
...
export default App;
```
###  2.15. <a name='ReadingAdditionalresourcesforReactcomponentsandwheretheylive'></a>ReadingAdditional resources for React components and where they live
...
- [Visual Studio Code keyboard shortcuts (macOS)](https://code.visualstudio.com/shortcuts/keyboard-shortcuts-windows.pdf)
